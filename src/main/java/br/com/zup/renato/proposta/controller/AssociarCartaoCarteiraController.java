...
public class AssociarCartaoCarteiraController {
	...
	public ResponseEntity<?> associarCartao(@PathVariable String id, UriComponentsBuilder uriComponentsBuilder,
			@RequestBody AssociarCarteiraForm associarCarteiraForm){
		...
		CarteiraSend carteiraSend = new CarteiraSend(cartao.get().getProposta().getEmail(), associarCarteiraForm.getCarteira());
		//CarteiraSend carteiraSend = new CarteiraSend(null, null);
		try {
			CarteiraResponse carteiraResponse = cartoesClient.associarCartao(cartao.get().getIdCartao(), carteiraSend);
			Carteira carteira = new Carteira(carteiraResponse.getId(), associarCarteiraForm.getCarteira(), cartao.get()); 
			carteiraRepository.save(carteira);
			return ResponseEntity
					.created(uriComponentsBuilder.path("/carteira/{id}").buildAndExpand(carteira.getIdCarteira()).toUri())
					.build();
		}catch (Exception e) {
			//e.printStackTrace();
